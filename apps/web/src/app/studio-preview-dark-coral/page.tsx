...
// Protótipo isolado — modo DARK do sistema, accent unificado em coral.
// Par do studio-preview (modo light). Dados fictícios. Não substitui tela real.

const creations = [
  {
    title: 'IA não substitui advogado. Mas o advogado que usa IA substitui o que não usa.',
    platform: 'LinkedIn',
    grad: 'from-[#FF8A5B] to-[#C24A2E]',
    reach: '2.4k',
    rate: '6.1%',
  },
  {
...
export default function StudioPreviewDarkCoral() {
  return (
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-[#0B0D12]/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-9 w-9 rounded-full bg-gradient-to-br from-[#FF8A5B] to-[#C24A2E]" />
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 pb-24">
        <section className="pt-14 pb-10">
          ...
          <h1 className="max-w-3xl text-5xl font-bold leading-[1.05] tracking-tight md:text-6xl">
            Bora criar algo que{' '}
            <span className="bg-gradient-to-r from-[#FF6B4A] to-[#FFB088] bg-clip-text text-transparent">
              valha o feed
            </span>
            , Tales?
          </h1>
        ...
        <section className="grid gap-5 md:grid-cols-3">
          <button className="group relative col-span-2 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#1A1D27] to-[#0B0D12] p-8 text-left shadow-2xl shadow-black/40 transition-transform duration-300 hover:-translate-y-1">
            <div className="absolute -right-10 -top-10 h-44 w-44 rounded-full bg-[#FF6B4A]/15 blur-2xl" />
            <span className="inline-flex items-center gap-2 rounded-full bg-[#FF6B4A]/15 px-3 py-1 text-xs font-semibold text-[#FF6B4A]">
              ✦ Comece agora
            </span>
            ...
            <span className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#FF6B4A] px-5 py-2.5 text-sm font-semibold text-white transition-transform group-hover:gap-3">
              Criar com IA →
            </span>
          </button>
            ...
            {creations.map((c) => (
              ...
              >
                ...
                <div className="flex items-center justify-between px-5 py-4">
                  ...
                  <span className="text-white/30 transition-colors group-hover:text-[#FF6B4A]">↻</span>
                </div>
              </article>
            ))}
          </div>
        ...
        <section className="mt-16 grid gap-5 md:grid-cols-3">
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#1A1D27] to-[#0B0D12] p-7 md:col-span-2">
            ...
            <button className="mt-5 rounded-full bg-[#FF6B4A] px-5 py-2.5 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5">
              Criar parecido
            </button>
          </div>
          <div className="flex flex-col justify-center rounded-3xl border border-white/10 bg-white/[0.03] p-7">
            ...
            <div className="mt-4 flex gap-1.5">
              {[1, 1, 1, 1, 0].map((on, i) => (
                <div
                  key={i}
                  className={`h-1.5 flex-1 rounded-full ${on ? 'bg-[#FF6B4A]' : 'bg-white/10'}`}
                />
              ))}
            </div>
          </div>
