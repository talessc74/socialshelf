...
describe('GenerateContentPage', () => {
  ...
  it('narra a estimativa de tempo durante a geração e o total acumulado ao concluir, sumindo sozinha', async () => {
    mockedApi.getConnections.mockResolvedValue([makeConnection(Platform.LINKEDIN)])
    let resolveGenerate!: (value: ApiGenerationRequest) => void
    mockedApi.generateContent.mockReturnValue(
      new Promise((resolve) => {
    ...
    // Durante a geração: estimativa de tempo manual (1 plataforma + 1 card padrão = 11min).
    await waitFor(() => {
      expect(screen.getByTestId('selfie-narration')).toHaveTextContent('Isso levaria uns 11 min')
    })
    ...
    try {
      ...
      // Ao concluir: mensagem de recompensa com o total acumulado (mesmos 11min, primeira geração).
      expect(screen.getByTestId('selfie-narration')).toHaveTextContent('Pronto!')
      expect(screen.getByTestId('selfie-narration')).toHaveTextContent('Total: 11 min')

      // Some sozinha depois de ~5s.
      act(() => {
  ...
  it('não narra recompensa quando a geração falha — limpa a narração', async () => {
    mockedApi.getConnections.mockResolvedValue([makeConnection(Platform.LINKEDIN)])
    mockedApi.generateContent.mockRejectedValue(new Error('Erro genérico de geração'))
    ...
    // Estado final após a falha: erro mostrado, sem recompensa narrada.
    expect(await screen.findByText(/Erro genérico de geração/)).toBeInTheDocument()
    expect(screen.getByTestId('selfie-narration')).toHaveTextContent('')
  })
