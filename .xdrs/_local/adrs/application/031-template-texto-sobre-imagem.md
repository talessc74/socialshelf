...
**Seletor de estilo nunca oferecia "sem texto" (2026-06-21)**

A seção anterior tornou a barra de texto opcional no back-end (`hasTextOverlay`/checagem de headline vazio), mas esse caminho era inalcançável na prática: o Gemini sempre gera um headline não vazio por slide, e o seletor de estilo em `/dashboard/generate` (`TEMPLATE_STYLE_OPTIONS`) só oferecia 3 variações de "onde fica a barra" (`bold-bottom`, `centered-overlay`, `top-strip`) — nenhuma comunicava "sem texto algum". Deliberação convocou COMPASS · EMPIRICUS (UX) e AETHER · NEXUS (Design): a interface afirmava uma obrigação que o back-end já não tinha mais, e fotos fortes/autoexplicativas não tinham como ser publicadas sem ganhar uma faixa colorida por cima.

Decisão: novo valor de catálogo `TemplateStyle.NO_TEXT` ('no-text'). É uma escolha visível e deliberada do usuário — não um efeito colateral invisível de o Gemini "esquecer" de gerar headline. `SharpTemplateRenderer.render()` pula o overlay de texto quando `style === NO_TEXT`, além da checagem de headline vazio já existente. `GenerateContentUseCase`/`EditArtifactUseCase` computam `hasTextOverlay` considerando os dois critérios (`style !== NO_TEXT && headline.trim().length > 0`), então o Imagen também deixa de receber a instrução de "zona calma" quando o estilo é `NO_TEXT` — não há zona a reservar se não há barra. No front-end, `TEMPLATE_STYLE_OPTIONS` ganha o 4º card "Sem texto"; visualmente ele é só o swatch cinza-claro sem nenhuma barra/overlay sobreposta (nenhum JSX condicional precisou ser adicionado — a ausência de barra já é o estado "nenhuma das condições anteriores bateu"), sinalizando a diferença pela ausência, não por um elemento extra. Grid do seletor passou de `grid-cols-3` para `grid-cols-2 sm:grid-cols-4` para acomodar o 4º card sem espremer em telas pequenas.
