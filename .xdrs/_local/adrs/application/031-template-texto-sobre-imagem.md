---
name: _local-adr-policy-031-template-de-texto-sobre-imagem
...
# _local-adr-policy-031: Template de Texto Sobre Imagem
...
**Texto nunca vem do Imagen; imagem nasce do mesmo contexto do texto (2026-06-21)**

Duas falhas de produção corrigidas juntas: (1) a imagem de fundo nascia da descrição crua do usuário, nunca do headline real gerado pelo Gemini — fotos fracamente relacionadas a cada slide; (2) a faixa de texto era desenhada incondicionalmente, mesmo sem necessidade.

Deliberação (AETHER · NEXUS · COMPASS · EMPIRICUS · SCOUT) convergiu: texto para o usuário final nunca é desenhado pelo Imagen — geração de imagem não renderiza texto em português de forma confiável (classe de defeito já observada duas vezes). Decisão:

- `CopyGenerationResult` ganha `visualBriefs: string[]` — uma cena em inglês por slide, derivada do headline daquele slide, não da descrição genérica. `GenerateContentUseCase`/`EditArtifactUseCase` chamam `ImageGeneratorPort.generateImage({ description: visualBriefs[i], ... })`, nunca `input.description`.
- `hasTextOverlay` = `style !== NO_TEXT && headline.trim().length > 0`. Quando true, `ImagenImageGenerator` pede uma área "calma" na zona do `templateStyle` escolhido — instrução puramente fotográfica, sem mencionar texto, legenda ou barra: mencionar "espaço reservado para texto" faz o Imagen desenhar texto fictício ilegível (bug já observado em produção — o modelo interpreta a cena como peça publicitária com `copy space`). `negativePrompt` reforça com `lorem ipsum`/`placeholder text`/`gibberish text`.
- `SharpTemplateRenderer.render()` só desenha a faixa quando há headline não vazio **e** `style !== NO_TEXT` — sem as duas condições, o card final é só foto + selo do logo.
- Opção de o Imagen desenhar texto integrado à imagem não foi implementada — risco de confiabilidade descartado por SCOUT; revisitar só se houver modelo com renderização de texto em português comprovadamente confiável.

**Estilo "sem texto" no catálogo (2026-06-21)**

O seletor de estilo só oferecia 3 variações de onde fica a faixa — nenhuma comunicava "sem texto", tornando o `hasTextOverlay` opcional do back-end inalcançável na prática (Gemini sempre gera headline não vazio). Deliberação (COMPASS · EMPIRICUS · AETHER · NEXUS) adicionou `TemplateStyle.NO_TEXT` ('no-text') como escolha visível e deliberada do usuário. `SharpTemplateRenderer` pula o overlay quando `style === NO_TEXT`; front-end ganha o 4º card "Sem texto", grid ajustado de `grid-cols-3` para `grid-cols-2 sm:grid-cols-4`.
