...
**Remover foto individual da campanha (2026-07-11)** — depois de testar o upload repetidas vezes durante a investigação do bug do índice, o usuário acumulou 148 fotos na mesma campanha de teste (várias tentativas duplicadas) e perguntou como remover as que não quer, sem opção nenhuma na tela pra isso. Adicionado `DELETE /campaigns/:id/photos/:photoId`: `CampaignPhotoRepository.delete` (novo método no port, implementado em `FirestoreCampaignPhotoRepository`) busca e apaga o documento em uma operação, devolvendo a foto apagada (ou `null` se já não existir, → 404) pra que o use-case saiba o `storagePath` a limpar no storage. `DeleteCampaignPhotoUseCase` chama um novo endpoint interno `POST /images/delete` no generator-service (mesmo padrão de auth por `X-Internal-Secret` de `/images/upload`; só passou a existir agora porque `ImageStoragePort.delete` já existia mas nunca tinha rota HTTP pra ele) — essa limpeza de storage é best-effort (falha é ignorada) porque o documento do Firestore, que é o que toda tela realmente lê, já foi removido antes; deixar um blob órfão no bucket é um custo bem menor que um 500 numa ação que já teve seu efeito visível concluído. Na tela de upload, cada miniatura ganhou um botão "×" sempre visível (não só no hover, pra funcionar em touch) que pede confirmação (`confirm()`, mesmo padrão já usado para cancelar agendamento em `/dashboard/scheduled`) antes de apagar.

## What this does not solve

Pausar/retomar campanha em andamento — ativar hoje é definitivo, sem botão de pausa (`PhotoCampaign.status` não tem um estado `'paused'`). Detecção de fotos duplicadas/quase-iguais (hash perceptual) — nenhuma foto é sinalizada nesta fase, mesmo que sejam idênticas; a remoção adicionada nesta rodada é manual, uma foto de cada vez, não uma sugestão automática de duplicatas. Legenda automática via IA — o texto inicial é um template simples (`nome/descrição + hashtags das palavras-chave`), sem chamar Gemini. Notificação por e-mail — bloqueado até confirmação de credencial de um provedor (Resend ou outro); hoje o produto não envia e-mail algum, então isso não é só configurar um lembrete, é a primeira peça de infraestrutura de notificação. Edição de agrupamento de carrossel por item na tela de revisão — hoje só a legenda é editável ali; trocar quais fotos formam qual item exige editar via API diretamente.
