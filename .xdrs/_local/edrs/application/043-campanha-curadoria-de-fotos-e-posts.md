...
**Reordenar na tela de upload: arraste de verdade via Pointer Events, não setas (2026-07-11)**

Primeira versão usava um par de botões "‹"/"›" por miniatura (mesmo raciocínio de robustez mobile-first do botão de apagar foto — sempre visível, não só no hover). O usuário perguntou por que não simplesmente arrastar com o mouse ou o dedo. Resposta trocada por implementação: `HTMLElement.setPointerCapture` (Pointer Events, não HTML5 drag-and-drop nativo, que não funciona direito em touch sem polyfill) unifica mouse e toque no mesmo conjunto de handlers, sem precisar de nenhuma lib nova. `onPointerDown` numa miniatura captura o ponteiro nela; a cada `onPointerMove`, a miniatura arrastada troca de posição com a mais próxima do cursor (distância euclidiana entre os centros via `getBoundingClientRect`, recalculada a cada miniatura); `onPointerUp` persiste a ordem final via `PUT /campaigns/:id/photos/order`. O cache do React Query é atualizado direto no `onSuccess` da mutação (`setQueryData`, não `invalidateQueries`) pra evitar a grade "piscar" de volta pra ordem antiga por um instante até o refetch chegar — o próprio arraste que acabou de acontecer não pode parecer que desfez sozinho. `touch-none` (CSS `touch-action: none`) na miniatura impede o navegador de interpretar o gesto como rolagem da página em vez de arraste. jsdom (ambiente de teste) não implementa a Pointer Capture API — `vitest.setup.ts` ganhou stubs de `setPointerCapture`/`releasePointerCapture`/`hasPointerCapture`, senão qualquer teste que dispara `pointerdown` numa miniatura quebra com "not a function".

A tela de linha do tempo continua com os botões "‹"/"›" — arrastar entre containers diferentes (cada post é um `<li>` separado) é um problema mais complexo (sortable multi-lista) que arrastar dentro de uma grade única; fica como possível evolução futura, não implementado nesta rodada.
